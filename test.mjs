...
function formatTimeMMDDYY(d = new Date()) {
  const mm = String(d.getMonth() + 1).padStart(2, '0');
  const dd = String(d.getDate()).padStart(2, '0');
  const yy = String(d.getFullYear()).slice(-2);
  // Return mm-dd-yy as requested (no time component)
  return `${mm}-${dd}-${yy}`;
}

async function moveFilesToModelFolder(files, m, quality, aspectRatio, promptText, storageChoice, result) {
  const safeName = (m.name || m.id || 'model').replace(/[^a-zA-Z0-9-_]/g, '_');
  const baseModelDir = path.join('./modelImage', safeName);
  const nsfwDir = path.join(baseModelDir, 'nsfw');
  ensureDir(baseModelDir);
  ensureDir(nsfwDir);

  const manifestPath = path.join(baseModelDir, 'manifest.json');
  let manifest = [];
  ...
  for (let i = 0; i < files.length; i++) {
    const src = files[i];
    const timeTag = formatTimeMMDDYY(new Date());
    const qualityTag = (quality.name || 'Fast').replace(/\s+/g, '');
    const sizeTag = `${aspectRatio.w}x${aspectRatio.h}`;
    const ext = path.extname(src) || '.jpeg';
    const idx = i + 1;
    const folder = (storageChoice === 'nsfw') ? nsfwDir : baseModelDir;
    const baseTarget = path.join(folder, `${safeName}_${timeTag}_${qualityTag}_${sizeTag}_${idx}${ext}`);
    const target = ensureUniqueFilename(baseTarget);
    try {
      fs.renameSync(src, target);
      console.log('Saved ->', target);
      manifest.push({ file: path.basename(target), prompt: promptText, model: m, quality: quality.name, size: sizeTag, timings: result.timings || {}, created_at: timeTag, index: idx, storage: storageChoice });
    } catch (e) {
      console.error('Failed to move file', src, '->', target, e.message);
    }
  }

  try { fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), 'utf8'); } catch (e) { console.error('Failed to write manifest', e.message); }
}

async function generateForAllModels(promptText, qualityName, context, storageChoice = 'normal') {
  console.log('Generate for all models:', promptText, '| quality:', qualityName, '| save:', storageChoice);
  const quality = Object.values(API_MAPPINGS.QUALITY).find(q => q.name.toLowerCase() === (qualityName || '').toLowerCase()) || API_MAPPINGS.QUALITY[1];
  const aspectRatio = API_MAPPINGS.ASPECT_RATIO[3]; // default 1:1
  const models = Object.values(API_MAPPINGS.MODELS);
  ensureDir('./modelImage');

  for (const m of models) {
    console.log('\n=== Model:', m.name, 'id=', m.id, '===');
    const settings = { prompt: promptText, model: m, quality, aspectRatio };
    let attempts = 0;
    let result = null;
    while (attempts < 3) {
      attempts++;
      console.log(`Generating for ${m.name} (attempt ${attempts})`);
      result = await autoGenerate(settings, context);
      if (result?.savedFiles && result.savedFiles.length > 0) break;
      console.log(`No files produced for ${m.name} (attempt ${attempts})`);
      if (attempts === 1) await wait(5000); else await wait(15000);
      // refresh auth if JWT changed
      if (result && result.jwt) context.jwt = result.jwt;
    }

    const files = (result && result.savedFiles) || [];
    if (files.length > 0) {
      await moveFilesToModelFolder(files, m, quality, aspectRatio, promptText, storageChoice, result);
    } else {
      console.log(`No files to move for ${m.name}`);
    }
  }
  console.log('\nAll models processed. Manifests are in ./modelImage/<ModelName>/manifest.json');
}
...
async function autoGenerate(settings, context) {
  ...
  try {
    ...
    timings.saved = saved;
    timings.urls = urls;
  return { urls, timings, jwt, runId, savedFiles };

  } catch (e) {
    console.error('Generation failed:', e.message);
    return { urls: [], timings: { error: e.message }, jwt, runId, savedFiles: [] };
  }
}
...
async function getUserInput() {
  ...
  // Ask where to save the generated images for this prompt
  let saveChoice = (await rl.question('\nSave location: 1) normal  2) nsfw secret  (enter 1 or 2) [1]: ')).trim();
  if (saveChoice === '') saveChoice = '1';
  rl.close();
  const storage = (saveChoice === '2') ? 'nsfw' : 'normal';
  return { prompt, model, quality, aspectRatio: ratio, storage };
}
...
(async () => {
  try {
    ...
    // Choose mode: interactive prompts or batch generate for all models
    const mainRl = createInterface({ input: process.stdin, output: process.stdout });
    const mode = (await mainRl.question('Mode: 1) interactive  2) generate all models  (enter 1 or 2): ')).trim();
    if (mode === '2') {
      const promptText = (await mainRl.question('Prompt to use for all models: ')) || 'a magical cat wizard';
      const qualityChoice = (await mainRl.question('Quality (Fast or High Quality) [Fast]: ')) || 'Fast';
      const saveChoice = (await mainRl.question('Save location for all models: 1) normal  2) nsfw secret  (enter 1 or 2) [1]: ')) || '1';
      mainRl.close();
      const storage = (String(saveChoice).trim() === '2') ? 'nsfw' : 'normal';
      await generateForAllModels(promptText, qualityChoice, { page, browser, jwt }, storage);
      console.log('Batch generation complete — closing browser.');
    ...
    while (true) {
      ...
      // Move saved files into model folder according to user's storage choice from getUserInput()
      const files = result?.savedFiles || [];
      if (files.length > 0) {
        try {
          await moveFilesToModelFolder(files, settings.model, settings.quality, settings.aspectRatio, settings.prompt, settings.storage, result);
        } catch (e) {
          console.error('Failed to move generated files:', e.message || e);
        }
      } else {
        console.log('No files saved for this generation.');
      }
